        ...
        for (let kopel of celled) {
            if (kopel.isGroup == false) {
                if (kopel.status == "offer") {
                    let nomer = await zex.sendTextWithMentions(
                        kopel.from,
                        `*${zex.user.name}* tidak bisa menerima panggilan ${kopel.isVideo ? `video` : `suara`}. Maaf @${kopel.from.split("@")[0]} kamu akan diblokir. Silahkan hubungi Owner membuka blok !`,
                    );
                    zex.sendContact(
                        kopel.from,
...
`;
    ...
    //=================================================//
    zex.sendText = (jid, text, quoted = "", options) =>
        zex.sendMessage(jid, { text: text, ...options }, { quoted });

    //=================================================//
    zex.sendTextWithMentions = async (jid, text, quoted, options = {}) =>
        zex.sendMessage(
            jid,
            {
                text: text,
    ...
    //=================================================//
    zex.downloadAndSaveMediaMessage = async (
        message,
        filename,
    ...
    zex.copyNForward = async (
        ...
        await zex.relayMessage(jid, waMessage.message, {
