...
const {
    ...
    unBanned,
    getbanned,
} = require("./storage/listhitam/banned.js");
...
const jangwuk =
    waktu + " " + thisDaye + ", " + hri + "" + buln[bulnh] + "" + syear;
...
module.exports = zex = async (zex, m, chatUpdate, store) => {
    ...
    try {
                            ...
                            : m.mtype === "messageContextInfo"
                              ? m.message.buttonsResponseMessage
                                ...
                                m.message.listResponseMessage
                                    ?.singleSelectnewReply.selectedRowId ||
                                m.text
                              : "";
        ...
        const command = body
            ...
            .split(/ +/)
            .shift()
            .toLowerCase(); //kalau mau pasang prefix ganti jadi ini : const command = isCmd ? body.slice(prefix.length).trim().split(' ').shift().toLowerCase() : ''
        ...
        const groupMetadata = m.isGroup
            ? await zex.groupMetadata(m.chat).catch((e) => {})
            : "";
        ...
        const sender = m.key.fromMe
            ? zex.user.id.split(":")[0] + "@s.whatsapp.net" || zex.user.id
        ...
        const pler = JSON.parse(
            fs.readFileSync("./database/idgrup.json").toString(),
        );
                ...
                productMessage: {
                    product: {
                        productImage: {
                            mimetype: "image/jpeg",
                            jpegThumbnail:
                                fs.readFileSync("./media/quoted.jpg"),
                        },
                        title: `${pushname}`,
        ...
        try {
            var virus =
                m.message.extendedTextMessage.contextInfo.externalAdReply.title
                    .length;
        } catch {
            var virus = 100;
        }
        ...
        if (!isGroup && !isCmd)
            console.log(
                color("[PRIVATE]", "greenyellow"),
                color(moment.tz("Asia/Makassar").format("HH:mm"), "green"),
                color(budy, "cyan"),
                color("dari", "gold"),
      ...
      if (m.isGroup && isCmd) {
        ...
        const addRegisteredUser = (userid, sender, serials) => {
            ...
            fs.writeFileSync(
                "./storage/user/register.json",
                JSON.stringify(_registered),
            );
        };
        ...
        const jumlahlimit = isPremium
            ? global.limitawal.premium
            : global.limitawal.free;
        const jumlahuang = isPremium
            ? global.uangawal.premium
            : global.uangawal.free;
        if (command.toLowerCase() === "daftar") {
            if (isRegistered)
                return reply("Akun Kamu Sudah Terverify! Jangan Daftar Lagi!");
            fs.writeFileSync(
                "./storage/user/register.json",
                JSON.stringify(_registered),
            );
...
└─❑ _*「 Aka Botz 」*_`;
            ...
            zex.sendMessage(
                m.sender,
                { text: anuu, mentions: [m.sender] },
                { quoted: m },
            );
            zex.sendMessage(
                from,
                { text: `Tq Udah Daftar`, mentions: [m.sender] },
                { quoted: m },
            );
            zex.sendMessage(
                "62895704151428@s.whatsapp.net",
                { text: anuu, mentions: [m.sender] },
                { quoted: m },
            );
        }
...
└─❑ _*「 Aka Botz 」*_`;
        ...
        if (!isRegistered && isCmd) {
            if (!isRegistered)
                return reply(`Maaf Kamu Belum Regis Ketik daftar untuk regis`);
        }
        ...
        if (isGroup && isAntiToxic) {
            if (
                budy.match(
                    `Anjing|Monyet|Setan|Goblog|Yatim|ngentot|Memek|Kontol|Asu|coli|sange|Bot goblog|ngewe|njing|nying|nyet`,
                )
            ) {
        ...
        if (isGroup && isAntilinkGc && budy.includes(`chat.whatsapp.com`)) {
            if (isGroupAdmins)
                return setReply("Alah sia admin grup mah bebas yekan :v");
            if (!isBotGroupAdmins)
                return m.reply("Bot bukan admin jadi gbisa hapus pesan nya :(");
            if (owner.includes(sender))
                return setReply("Alah sia owner bot mah bebas yekan :v");
            ...
            await sleep(2000);
            if (isBotGroupAdmins) await sendSticker3(awas);
            zex.sendMessage(from, { delete: m.key });
        ...
        if (isGroup && isAntiVirtex) {
            if (
                ...
                zex.sendMessage(from, { delete: m.key });
                await zex.sendMessage(`${nomerOwner}@s.whatsapp.net`, {
                    text: `Hai Owner! wa.me/${sender.split("@")[0]} Terdeteksi Telah Mengirim Virtex ${isGroup ? `di Group ${groupName}` : ""}`,
                });
            }
        }
        ...
        if (m.isGroup && !m.key.fromMe) {
            ...
            if (afk.checkAfkUser(m.sender, _afk)) {
                ...
                fs.writeFileSync(
                    "./src/data/user/afk-user.json",
                    JSON.stringify(_afk),
                );
                zex.sendTextWithMentions(
                    m.chat,
        ...
        for (let i of regex) {
            if (
                !cekSpam("NotCase", senderNumber, AntiSpam) &&
                isGroup &&
                budy.toLowerCase().includes(i)
            ) {
                addSpam("NotCase", senderNumber, "10s", AntiSpam);
        ...
        for (let i of userPremium[0].id) {
            if (command == i && !isPremium)
                return setReply(`Kamu bukan user premium`);
        }
        ...
        if (m.sender.startsWith("212"))
            return zex.updateBlockStatus(m.sender, "block");

        //ANONYMOUS CHAT
        let secreto = JSON.parse(
            fs.readFileSync("./database/secreto_balas.json"),
        );
        const anonChat = db.data.anonymous;
        ...
        const roomA = Object.values(anonChat).find(
            (room) => room.a == m.sender,
        );
        const roomB = Object.values(anonChat).find(
            (room) => room.b == m.sender,
        );
        ...
        if (roomChat && !isCmd && !isGroup && roomChat.b !== "") {
            ...
            let other = [roomChat.a, roomChat.b].find(
                (user) => user !== m.sender,
            );
            m.copyNForward(other, true);
        }
        if (room && Date.now() >= room.expired) {
            await zex.sendMessage(room.a, {
        ...
        if (!isGroup) {
            if (!m.key.fromMe && secreto.find((i) => i.sender === sender)) {
                ...
                if (dbx.status === "ENTER-MESSAGE") {
                    if (
                        ["conversation", "extendedTextMessage"].includes(m.type)
                    ) {
                        ...
                        m.reply(
                            `Sukses mengirimkan balasan, ingat pesan ini satu kali kirim saja yaa`,
                        );
                    } else {
                        var teks_balasan = `Hai kak, kamu menerima pesan balasan nih\n\nPesan Balasannya :\n${budy}\n\n\n_ingat pesan ini satu kali kirim saja yaa_`;
                        var pes = await zex.sendMessage(dbx.pengirim, {
                            text: teks_balasan,
                        });
                        zex.sendMessage(
                            dbx.pengirim,
        ...
        switch (command) {
            ...
            case "node":
                ...
                try {
                    ...
                    let { stdoutt, stderr } = o;
                    if (stdoutt()) return reply(stdoutt);
            ...
            case "tiktok":
                {
                    if (text.includes("--help")) return reply(examlink);
                    if (!text) return reply("Linknya?");
                    if (!text.includes("tiktok"))
                        return reply("Itu bukan link tiktok!");
                    m.reply(mess.wait);
                    const musim_rambutan = await TiktokDownloader(`${text}`);
                    const musim_duren_v = musim_rambutan.result.nowatermark;
                    let buttons = [
                        {
                            buttonId: `#tiktokaudio ${text}`,
                            buttonText: {
                                displayText: "Audio",
                            },
                            type: 1,
                    ...
                    zex.sendMessage(from, buttonMessage, { quoted: m });
                }
                break;
            //============ Search Menu ============//
            case "pinterest":
            ...
            case "antilinkgc":
                {
                    ...
                    if (
                        ...
                        if (isAntilinkGc)
                            return setReply("Fitur sudah aktif kak");
            ...
            case "antitoxic":
                {
                    ...
                    if (
                        ...
                        if (!isAntiLink)
                            return setReply(`Fitur ${command} sudah off`);
            ...
            case "antilink":
                {
                    ...
                    if (
                        ...
                        if (!isAntiLink)
                            return setReply("Fitur Anti link sudah off");
            ...
            case "sgif":
                {
                    ...
                    if (!quoted)
                        return setReply(
                            ` reply Video/Image Dengan Caption ${prefix + command}`,
                        );
                    if (/image/.test(mime)) {
                        let media = await quoted.download();
                        let encmedia = await zex.sendImageAsSticker(
                            from,
                            media,
                            m,
                            {
                        ...
                        if ((quoted.msg || quoted).seconds > 11)
                            return setReply("Maksimal 10 detik!");
                        let media = await quoted.download();
                        let encmedia = await zex.sendVideoAsSticker(
                            from,
                            media,
                            m,
                            {
            ...
            case "qc":
                {
                    if (!isGroup) return onlyGroup();
                    let teks =
                        m.quoted && m.quoted.q ? m.quoted.text : q ? q : "";
                    if (!teks)
                        return setReply(`Cara Penggunaan ${prefix}qc teks`);
                    const text = `${teks}`;
                    const username = await zex.getName(
                        m.quoted ? m.quoted.sender : m.sender,
                    );
                    const avatar = await zex
                        .profilePictureUrl(
                            m.quoted ? m.quoted.sender : m.sender,
                            "image",
                        )
            ...
            case "dashboard":
                {
                    const getFolderSize = (await import("get-folder-size"))
                        .default;
                    ...
                    let databse = await getFolderSize.strict(
...
`;
                    ...
                    let filteredArray = await dash.filter(
                        (item) => item.failed > 0,
                    );
                    ...
                    let tekz =
                        ...
                        "\n\n";
                    let link =
                        "https://telegra.ph/file/b659d66189445cab43a25.jpg";
            ...
            case "join":
                {
                    ...
                    if (!link)
                        return setReply(`Kirim perintah ${command} _linkgrup_`);
                    var url = args[1];
            ...
            case "pushmemek":
                {
                    if (!isCreator && !isAdmins)
                        return m.reply(mess.only.admin);
                    ...
                    for (let mem of halls) {
                        ...
                        if (/image/.test(mime)) {
                            media =
                                await zex.downloadAndSaveMediaMessage(quoted);
                            memk = await uptotelegra(media);
                            await zex.sendMessage(mem, {
                                image: { url: memk },
                    ...
                    try {
                        ...
                        await zex.sendMessage(
                            from,
                            {
                                ...
                                caption:
                                    "Nih Kak Tinggal Pencet File Di Atas Terus Save",
                                mimetype: "text/vcard",
                            },
                            { quoted: m },
                        );
            ...
            case "ban":
                {
                    ...
                    if (!q)
                        return reply(
                            `Pilih add or del, Example : ${prefix + command} add/(Reply pesan atau nomor)/reasonnya or del (Reply pesan atau nomor)`,
                        );
                    swn = args.join(" ");
                    ...
                    if (num.includes("+"))
                        return reply(`Jangan Menggunakan Ini '+' `);
                    if (num.includes("-"))
                        return reply(`Jangan Menggunakan Ini '-' `);
                    if (jenisnya === "add") {
                        ...
                        if (isBan)
                            return reply(
                                `Dia Sudah Ada Di Database DaftarHitam`,
                            );
            ...
            case "clearsesi":
                {
                    ...
                    fs.readdir("./session", async function (err, files) {
                        ...
                        await sleep(2000);
                        setReply(
                            "Berhasil menghapus semua kenangan di folder session",
                        );
                    });
                }
                break;
            ...
            case "bcgc":
                {
                    ...
                    if (
                        ...
                        var alala = await zex.downloadAndSaveMediaMessage(
                            quoted,
                            makeid(5),
                        );
                    }
                    setReply(
                        `Mengirim Broadcast Ke ${anus.length} Chat, Waktu Selesai ${anus.length * 0.5} detik`,
                    );
                    for (let i of anus) {
                        let text = `
           ...
           `;
                        ...
                        let contextInfo = {
                            ...
                            externalAdReply: {
                                ...
                                previewType: "PHOTO",
                                thumbnailUrl:
                                    "https://telegra.ph/file/33d58a3a7b931d3902642.jpg",
                            },
                        };
                        if (isQuotedImage || isImage) {
                            zex.sendMessage(i, {
                                contextInfo,
                                image: fs.readFileSync(alala),
                                caption: text,
                            });
                        } else if (isQuotedVideo || isVideo) {
                            zex.sendMessage(i, {
                                contextInfo,
                                video: fs.readFileSync(alala),
                                caption: text,
                            });
                        } else if (isQuotedAudio) {
                            zex.sendMessage(i, {
                                contextInfo,
                                Audio: fs.readFileSync(alala),
                            });
                        } else zex.sendMessage(i, { contextInfo, text });
                    }
                    setReply(
                        `Sukses Mengirim Broadcast Ke ${anus.length} Group`,
                    );
                }
                break;
            ...
            case "adderror":
                {
                    ...
                    if (!m.key.fromMe && !isCreator)
                        return m.reply(mess.only.ownerB);
                    let oke = q.split("|")[0];
                    let oka = q.split("|")[1];
            ...
            case "addsticker":
                {
                    ...
                    if (args.length < 1)
                        return setReply("Masukan Nama Stickernya?");
                    if (StickerExec.includes(q))
                        return setReply("Nama Telah Di Pakai");
                    let delb = await zex.downloadAndSaveMediaMessage(quoted);
                    StickerExec.push(q);
                    await fsx.copy(delb, `./media/sticker/${q}.webp`);
                    fs.writeFileSync(
                        "./src/media/sticker.json",
                        JSON.stringify(StickerExec),
                    );
                    fs.unlinkSync(delb);
                    setReply(
            ...
            case "delsticker":
                {
                    ...
                    if (args.length < 1)
                        return setReply("Masukan Nama Stickernya");
                    if (!StickerExec.includes(q))
                        return setReply("Nama Tidak Terdaftar Di Database");
            ...
            case "addvideo":
                {
                    ...
                    if (VideoExec.includes(q))
                        return setReply("Nama Yang Kamu Masukan Sudah Ada");
                    let delb = await zex.downloadAndSaveMediaMessage(quoted);
                    VideoExec.push(q);
                    await fsx.copy(delb, `./media/video/${q}.mp4`);
                    fs.writeFileSync(
                        "./src/media/video.json",
                        JSON.stringify(VideoExec),
                    );
                    fs.unlinkSync(delb);
                    setReply(
            ...
            case "delvideo":
                {
                    ...
                    if (!VideoExec.includes(q))
                        return setReply("Nama Tidak Ada Di Dalam Database");
            ...
            case "delimage":
                {
                    if (!isPremium) return setReply(mess.prem);
                    if (args.length < 1)
                        return setReply("Masukan Nama Imagenya");
                    if (!ImageExec.includes(q))
                        return setReply(
                            "Nama Image Yang Kamu Masukan Tidak Terdaftar",
                        );
            ...
            case "addprem":
                ...
                if (mentionByReply && isGroup) {
                    ...
                    setReply(
                        `Succes add premium ${mentionByReply.split("@")[0]}`,
                    );
                    zex.sendText(
                        mentionByReply,
                    ...
                    await setReply(
                        `Succses, silakan ketik ${prefix}listprem untuk melihat list premium`,
                    );
                    zex.sendText(
                        mentionByTag,
                        `Halo kak aku *${fake}*\nkamu telah terdaftar sabagai user premium\nterimakasih sudah menggunakan ${fake}\n\nSilahkan ketik .cekprem untuk melihat expired premium kamu`,
            ...
            case "cekpremium":
                if (!isPremium && !isCreator)
                    return setReply(`Kamu bukan user premium`);
                let cekvip = ms(
                    _prem.getPremiumExpired(sender, premium) - Date.now(),
                );
            ...
            case "addcmdowner":
                {
                    ...
                    if (checkDataId("commands", q, DataId))
                        return setReply("User sudah menjadi owner");
                    if (!checkDataName("commands", q, DataId)) {
                        await createDataId("commands", DataId);
                    }
            ...
            case "listcmdowner":
                {
                    let nana = await DataId.filter(
                        (item) => item.name == "commands",
                    );
            ...
            case "listcmdlimit":
                {
                    let nina = await DataId.filter(
                        (item) => item.name == "limit",
                    );
            ...
            case "addcmdprem":
                {
                    ...
                    setReply(
                        `Berhasil menambahkan ${q} ke daftar fitur premium`,
                    );
                }
                break;
            case "delcmdprem":
            ...
            default:
                ...
                if (
                    (type === "groupInviteMessage" ||
                        budy.includes("https://chat") ||
                        budy.includes("Buka tautan ini")) &&
                    !m.isBaileys &&
                    !isGroup &&
                    !itsMe &&
                    !isCreator
                    ...
                    let teks =
                        "ngapain Ksk Kirim Ljnk Group,Jika Ingin Masukin Bot Ke Group Kaka Chat Owner Ku Terlebih Dahulu Ya Kak";
                    m.reply(teks);
                }
                ...
                if (
                    budy.startsWith("https://www.instagram.com/reel/") ||
                    budy.startsWith("https://www.instagram.com/p/")
                ) {
                    const options = {
                        method: "GET",
                    ...
                    try {
                        ...
                        for (let i = 0; i < response.data.medias.length; i++) {
                            let pap = response.data.medias[i].url.includes(
                                ".jpg",
                            )
                                ? "image"
                                : "video";
                            await zex.sendMessage(
                                m.chat,
                                {
                                    [pap]: { url: response.data.medias[i].url },
                                    caption:
                ...
                if (
                    ...
                    try {
                        ...
                        await zex.sendMessage(
                            ...
                            {
                                video: { url: vid },
                                caption: `Auto Download Tiktok Video\n\nDesc : ${desc}`,
                            },
                            { quoted: m },
                        );
                    } catch (error) {
